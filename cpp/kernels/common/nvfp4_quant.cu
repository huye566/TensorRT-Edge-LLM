...
#else
#define CUTLASS_ARCH_MMA_SM100A_ENABLED 0
#endif
...
inline int getMultiProcessorCount() {
  static int multi_processor_count = []() {
    ...
    CUDA_CHECK(cudaDeviceGetAttribute(&count, cudaDevAttrMultiProcessorCount, device_id));
...
void scaled_fp4_quant(
    ...
    if (n % 16 != 0) {
        throw std::runtime_error("The N dimension must be multiple of 16.");
    }
    ...
    switch (inputType) {
        case nvinfer1::DataType::kHALF: {
            auto input_ptr = reinterpret_cast<half const*>(input);
            invokeFP4Quantization(m, n, input_ptr, SFScale, output, SFOuput,
                                  useUE8M0, multiProcessorCount, stream);
            break;
        }
        case nvinfer1::DataType::kBF16: {
            auto input_ptr = reinterpret_cast<__nv_bfloat16 const*>(input);
            invokeFP4Quantization(m, n, input_ptr, SFScale, output, SFOuput,
                                  useUE8M0, multiProcessorCount, stream);
            break;
        }

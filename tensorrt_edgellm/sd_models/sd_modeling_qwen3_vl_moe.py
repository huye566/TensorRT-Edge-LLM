...
class Qwen3VLTextMoE(nn.Module):
    ...
    def forward_v1(self, x):
        ...
        #     print("MoE routing unique experts:", expert_idx.unique())
        #     self._printed = True

        out = torch.zeros_like(x)
        # if not hasattr(self, "_infer_stats"):
        #     self._infer_stats = True

        #     counts = torch.bincount(
        #         expert_idx.view(-1),
        #         minlength=self.num_experts
        #     )
        #     print(f"[MoE] expert token counts: {counts.tolist()}")
    ...
    def forward_v3(self, x):
        ...
        # print(x.shape, expert_idx.shape)
        mask = F.one_hot(expert_idx, num_classes=self.num_experts).to(x.dtype)
        combined_out = torch.zeros_like(x)
        for i, expert in enumerate(self.experts):
            m = mask[:, :, i].unsqueeze(-1)
            expert_out = expert(x)
            # print(m.shape, expert_out.shape)
            combined_out = combined_out + (expert_out * m)
    ...
    def forward_v4(self, x):
        ...
        # [b, num_experts, s, 2048] -> [b, s, num_experts, 2048]
        expert_outputs = expert_outputs.transpose(1, 2)
        combined_out = torch.sum(expert_outputs * mask, dim=2)
...
class Qwen3VLTextDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen3VLTextConfig, layer_idx: int):
        ...
        self.mlp = Qwen3VLTextMoE(config, num_experts=3)
...
@auto_docstring
class SDQwen3VLMoePreTrainedModel(PreTrainedModel):
...
class SDQwen3VLMoeVisionModel(SDQwen3VLMoePreTrainedModel):
...
class SDQwen3VLMoeTextModel(SDQwen3VLMoePreTrainedModel):
...
@auto_docstring
class SDQwen3VLMoeModel(SDQwen3VLMoePreTrainedModel):
    base_model_prefix = ""
    ...
    def __init__(self, config):
        super().__init__(config)
        self.visual = SDQwen3VLMoeVisionModel._from_config(config.vision_config)
        self.language_model = SDQwen3VLMoeTextModel._from_config(config.text_config)
...
class SDQwen3VLMoeForConditionalGeneration(SDQwen3VLMoePreTrainedModel, GenerationMixin):
    _checkpoint_conversion_mapping = {}
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = SDQwen3VLMoeModel(config)
...
__all__ = [
    "SDQwen3VLMoeVisionModel",
    "SDQwen3VLMoeForConditionalGeneration",
    "SDQwen3VLMoeModel",
    "SDQwen3VLMoePreTrainedModel",
    "SDQwen3VLMoeTextModel",
]

...
"""
Qwen3-VL moe visual model wrapper and export functionality.
...
from transformers.models.qwen3_vl_moe.modeling_qwen3_vl_moe import (
    Qwen3VLMoeVisionAttention, Qwen3VLMoeVisionBlock, Qwen3VLMoeVisionModel,
    apply_rotary_pos_emb_vision)

from ..onnx_export.onnx_utils import export_onnx


class Qwen3VLMoeVisionAttentionPatch(Qwen3VLMoeVisionAttention):
...
class QuantQwen3VLMoeVisionAttentionPatch(Qwen3VLMoeVisionAttentionPatch):
    def __init__(self, config: Any) -> None:
        super().__init__(config)
        self._setup()
...
mtq.register(original_cls=Qwen3VLMoeVisionAttentionPatch,
             quantized_cls=QuantQwen3VLMoeVisionAttentionPatch)


class Qwen3VLMoeVisionBlockPatch(Qwen3VLMoeVisionBlock):
    def __init__(self,
                 config: Any,
                 attn_implementation: str = "eager") -> None:
        super().__init__(config)
        self.attn = Qwen3VLMoeVisionAttentionPatch(config)
...
class Qwen3VLMoeVisionModelPatch(Qwen3VLMoeVisionModel):
    def __init__(self, config: Any) -> None:
        super().__init__(config)
        # Replace all blocks with patched versions
        self.blocks = nn.ModuleList([
            Qwen3VLMoeVisionBlockPatch(config, config._attn_implementation)
...
def export_qwen3_vl_moe_visual(
    model: Qwen3VLMoeVisionModelPatch,
    output_dir: str,

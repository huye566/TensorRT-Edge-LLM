...
void print_wrapper_results_table(const std::vector<TestResult>& results) {
    const int col1 = 15;  // 测试用例
    const int col2 = 10;  // 数据类型
    const int col3 = 18;  // 操作类型
    ...
    std::cout << "Cutlass Wrapper 性能测试结果汇总\n";
    std::cout << std::string(total_width, '=') << "\n";
...
int main(int argc, char** argv) {
    ...
    for (const auto& test_case : test_cases) {
        std::cout << "Testing GEMM: " << test_case.description << std::endl;
        TestResult result = benchmark_wrapper_gemm_half(test_case.M, test_case.N, test_case.K, iterations);
        all_test_results.push_back(result);
    }
    
    for (const auto& test_case : test_cases) {
        std::cout << "Testing GEMM+SiLU: " << test_case.description << std::endl;
        TestResult result = benchmark_wrapper_gemm_silu_half(test_case.M, test_case.N, test_case.K, iterations);
        all_test_results.push_back(result);
    }
    
    for (const auto& test_case : test_cases) {
        std::cout << "Testing GEMM+Bias: " << test_case.description << std::endl;
        TestResult result = benchmark_wrapper_gemm_bias_half(test_case.M, test_case.N, test_case.K, iterations);
        all_test_results.push_back(result);
    }
    
    // 打印结果
    print_wrapper_results_table(all_test_results);

...
void print_cublaslt_nvfp4_results_table(const std::vector<TestResult>& results) {
    ...
    std::cout << "CUBLASLT NVFP4 GEMM 性能测试结果汇总\n";
    std::cout << std::string(total_width, '=') << "\n";
...
int main(int argc, char** argv) {
    ...
    if (!cublaslt_wrapper.check_fp4_support()) {
        std::cout << "Warning: NVFP4 not supported on this hardware, running tests anyway..." << std::endl;
    }
    ...
    for (size_t i = 0; i < test_cases.size(); ++i) {
        ...
        for (const auto& test_type : test_types) {
            std::cout << "\n[" << (i+1) << "/" << test_cases.size() << "] "
                      << "测试 CUBLASLT NVFP4 " << test_type.first
                      << ": " << test_case.description
                      << " (M=" << test_case.M << ", N=" << test_case.N << ", K=" << test_case.K << ")"
                      << std::endl;

            try {
                CublasLtTestParams params = {
                    test_case.M,
                    test_case.N,
                    test_case.K,
                    iterations,
                ...
                TestResult result = benchmark_cublaslt_nvfp4_gemm_half(params);
    ...
    // 打印详细结果表格
    print_cublaslt_nvfp4_results_table(all_test_results);
    ...
    std::cout << "\n===================================================\n";
    std::cout << "CUBLASLT NVFP4 GEMM 测试完成!\n";
    std::cout << "总测试数: " << total_tests << "\n";
    std::cout << "通过测试: " << passed_tests << "\n";
    std::cout << "失败测试: " << (total_tests - passed_tests) << "\n";
    std::cout << "通过率: " << std::fixed << std::setprecision(1)

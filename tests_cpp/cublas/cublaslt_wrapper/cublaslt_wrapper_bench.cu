...
bool cublaslt_gemm_run(T* d_C,
                      ...
                      int N,
                      int K,
                      CublasLtGemmType gemm_type,
                      ComputeType compute_type,
                      cublasLtHandle_t handle,
                      cudaStream_t stream = 0) {
    switch (gemm_type) {
        case CublasLtGemmType::GEMM:
            return cublaslt_gemm<T, false, false>(handle, M, N, K, d_A, d_B, d_bias, d_C,
                                   MemoryFormat::ROW_MAJOR, compute_type, stream);
        case CublasLtGemmType::GEMM_SiLU:
            return cublaslt_gemm<T, true, false>(handle, M, N, K, d_A, d_B, d_bias, d_C,
                                        MemoryFormat::ROW_MAJOR, compute_type, stream);
        case CublasLtGemmType::GEMM_Bias:
            return cublaslt_gemm<T, false, true>(handle, M, N, K, d_A, d_B, d_bias, d_C,
                                        MemoryFormat::ROW_MAJOR, compute_type, stream);
    }
    return false;
}
...
TestResult benchmark_cublaslt_gemm(int M, int N, int K,
    ...
    if constexpr (std::is_same<T, float>::value) {
        ...
        result.operation = "cuBLASLt_GEMM_" + compute_type_str;
    } else if constexpr (std::is_same<T, __half>::value) {
        result.data_type = "FP16";
        result.operation = "cuBLASLt_GEMM_" + compute_type_str;
    } else {
        result.data_type = "UNKNOWN";
        result.operation = "cuBLASLt_GEMM";
    }
    ...
    // 初始化cuBLASLt
    auto& cublaslt_wrapper = CublasLtWrapper::instance();
    if (!cublaslt_wrapper.initialize()) {
        std::cerr << "Failed to initialize CublasLtWrapper" << std::endl;
        result.passed = false;
        return result;
    }
    cublasLtHandle_t handle = cublaslt_wrapper.handle();
    ...
    // 预热
    bool success = cublaslt_gemm_run<T>(d_C, d_A, d_B, nullptr, M, N, K,
                                       CublasLtGemmType::GEMM, compute_type, handle);
    if (!success) {
        std::cerr << "cuBLASLt GEMM预热失败" << std::endl;
        result.passed = false;
        return result;
    }
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        success = cublaslt_gemm_run<T>(d_C, d_A, d_B, nullptr, M, N, K,
                                      CublasLtGemmType::GEMM, compute_type, handle);
        CUDA_CHECK(cudaEventRecord(stop));
        CUDA_CHECK(cudaEventSynchronize(stop));

        if (!success) {
            std::cerr << "cuBLASLt GEMM第" << i << "次迭代失败" << std::endl;
            result.passed = false;
            break;
        }
    ...
    if constexpr (std::is_same<T, float>::value) {
        if (compute_type == ComputeType::TFLOAT32) {
            // TF32精度较低
            abs_tolerance = 1e-3;
            rel_tolerance = 1e-3;
        } else {
    ...
    result.passed = error_result.passed && success;
...
TestResult benchmark_cublaslt_gemm_silu(int M, int N, int K,
    ...
    if constexpr (std::is_same<T, float>::value) {
        result.data_type = "FP32";
        result.operation = "cuBLASLt_GEMM_SiLU_" + compute_type_str;
    } else if constexpr (std::is_same<T, __half>::value) {
        result.data_type = "FP16";
        result.operation = "cuBLASLt_GEMM_SiLU_" + compute_type_str;
    } else {
        result.data_type = "UNKNOWN";
        result.operation = "cuBLASLt_GEMM_SiLU";
    }
    ...
    // 预热
    bool success = cublaslt_gemm_run<T>(d_C, d_A, d_B, nullptr, M, N, K,
                                       CublasLtGemmType::GEMM_SiLU, compute_type, handle);
    if (!success) {
        std::cerr << "cuBLASLt GEMM+SiLU预热失败" << std::endl;
        result.passed = false;
        return result;
    }
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        if (!success) {
            std::cerr << "cuBLASLt GEMM+SiLU第" << i << "次迭代失败" << std::endl;
            result.passed = false;
            break;
        }
...
TestResult benchmark_cublaslt_gemm_bias(int M, int N, int K,
    ...
    if constexpr (std::is_same<T, float>::value) {
        result.data_type = "FP32";
        result.operation = "cuBLASLt_GEMM_Bias_" + compute_type_str;
    } else if constexpr (std::is_same<T, __half>::value) {
        result.data_type = "FP16";
        result.operation = "cuBLASLt_GEMM_Bias_" + compute_type_str;
    } else {
        result.data_type = "UNKNOWN";
        result.operation = "cuBLASLt_GEMM_Bias";
    }
    ...
    // 预热
    bool success = cublaslt_gemm_run<T>(d_C, d_A, d_B, d_bias, M, N, K,
                                       CublasLtGemmType::GEMM_Bias, compute_type, handle);
    if (!success) {
        std::cerr << "cuBLASLt GEMM+Bias预热失败" << std::endl;
        result.passed = false;
        return result;
    }
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    for (int i = 0; i < iterations; ++i) {
        CUDA_CHECK(cudaEventRecord(start));
        success = cublaslt_gemm_run<T>(d_C, d_A, d_B, d_bias, M, N, K,
                                      CublasLtGemmType::GEMM_Bias, compute_type, handle);
        CUDA_CHECK(cudaEventRecord(stop));
        CUDA_CHECK(cudaEventSynchronize(stop));

        if (!success) {
            std::cerr << "cuBLASLt GEMM+Bias第" << i << "次迭代失败" << std::endl;
            result.passed = false;
            break;
        }
...
// 显式实例化模板函数
template TestResult benchmark_cublaslt_gemm<float>(int M, int N, int K,
                                                  ComputeType compute_type, int iterations);
template TestResult benchmark_cublaslt_gemm<__half>(int M, int N, int K,
                                                   ComputeType compute_type, int iterations);

template TestResult benchmark_cublaslt_gemm_silu<float>(int M, int N, int K,
                                                       ComputeType compute_type, int iterations);
template TestResult benchmark_cublaslt_gemm_silu<__half>(int M, int N, int K,
                                                        ComputeType compute_type, int iterations);

template TestResult benchmark_cublaslt_gemm_bias<float>(int M, int N, int K,
                                                       ComputeType compute_type, int iterations);
template TestResult benchmark_cublaslt_gemm_bias<__half>(int M, int N, int K,
                                                        ComputeType compute_type, int iterations);

...
// 复用的结果打印函数
void print_cublas_results_table(const std::vector<TestResult>& results) {
    ...
    std::cout << "cuBLAS Wrapper 性能测试结果汇总\n";
    std::cout << std::string(total_width, '=') << "\n";
...
int main(int argc, char** argv) {
    ...
    std::cout << "========================================\n";
    std::cout << "cuBLAS Wrapper 测试 (FP32 & FP16)\n";
    std::cout << "========================================\n\n";
    
    // 测试每个接口 - FP32
    std::cout << "\n=== 测试 FP32 精度 ===\n";
    for (const auto& test_case : test_cases) {
        std::cout << "Testing cuBLAS GEMM FP32: " << test_case.description << std::endl;
        TestResult result = benchmark_cublas_gemm<float>(test_case.M, test_case.N, test_case.K, iterations);
        all_test_results.push_back(result);
    }
    
    for (const auto& test_case : test_cases) {
        std::cout << "Testing cuBLAS GEMM+SiLU FP32: " << test_case.description << std::endl;
        TestResult result = benchmark_cublas_gemm_silu<float>(test_case.M, test_case.N, test_case.K, iterations);
        all_test_results.push_back(result);
    }
    
    for (const auto& test_case : test_cases) {
        std::cout << "Testing cuBLAS GEMM+Bias FP32: " << test_case.description << std::endl;
        TestResult result = benchmark_cublas_gemm_bias<float>(test_case.M, test_case.N, test_case.K, iterations);
        all_test_results.push_back(result);
    }
    ...
    std::cout << "\n=== 测试 FP16 精度 ===\n";
    for (const auto& test_case : test_cases) {
        std::cout << "Testing cuBLAS GEMM FP16: " << test_case.description << std::endl;
        TestResult result = benchmark_cublas_gemm<__half>(test_case.M, test_case.N, test_case.K, iterations);
        all_test_results.push_back(result);
    }
    
    for (const auto& test_case : test_cases) {
        std::cout << "Testing cuBLAS GEMM+SiLU FP16: " << test_case.description << std::endl;
        TestResult result = benchmark_cublas_gemm_silu<__half>(test_case.M, test_case.N, test_case.K, iterations);
        all_test_results.push_back(result);
    }
    
    for (const auto& test_case : test_cases) {
        std::cout << "Testing cuBLAS GEMM+Bias FP16: " << test_case.description << std::endl;
        TestResult result = benchmark_cublas_gemm_bias<__half>(test_case.M, test_case.N, test_case.K, iterations);
        all_test_results.push_back(result);
    }
    
    // 打印结果
    print_cublas_results_table(all_test_results);
    std::cout << "\nAll cuBLAS wrapper tests completed!\n";
